...
Both mode and time_signature have an obvious statistical mode. The value of 1 for the variable mode represents songs in major keys, making up a super majority of the songs, and almost every song uses the time_signature of 4 with only a handful using 3.
...
We also utilized the Cramer's V and obtained values that suggest some kind of dependence between the independent and dependent variables.
...
In our model development strategy, we adopt a progressive approach, beginning with a logistic model and refining it through oversampling and variable selection techniques. We also explore models such as ridge and lasso regression, and K-nearest neighbors (KNN) to enhance predictive accuracy. Model performance is assessed through Type 1 Error, Sensitivity, and ROC curve analysis, while also considering discriminant analysis assuming normal distribution of key variables.
...
We started by considering the logistic regression, a model that belongs to the category of the generalized linear model. The logistic regression models the probability of an event to occur (in our case of a song to be nominated) by having the log-odds of the event to be a linear combination of the independent variables.
...
Linear discriminant analysis is a technique based on the Bayes theorem and finds a linear combination of variables that best separates the classes of the dependent variable.
...
Quadratic discriminant analysis is a generalization of LDA and does not assume that dependent variable have the same variance.
