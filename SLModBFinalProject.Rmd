...
At first, we took a look at the independent variables by examining their correlation, which is the measurement of association between two variables. This is essentially a measurement of how similar the information we learn from different variables are. The larger the correlation between independent variables (negative or positive), the higher their collinearity, meaning the information they each carry is similar. This means their use together is less beneficial due to redundancy and we should avoid the pairing. We would prefer to utilize pairings of variables that offer different information to our model.
...
Although measurements of association between categorical variables are hard to interpret, the Cramer's V test still informs us if there is full disassociation or full association between said variables. From our data, it can be seen that key/mode and key/time_signature are definitely not fully disassociated or fully associated. The same can be said about mode/time_signature, although the measured value was closer to 0.

#### ANOVA Test

Next, we looked for associations between each of the categorical variables and all of the continuous variables utilizing ANOVA. ANOVA (or ANalysis Of VAriation) offers a way to compare continuous and categorical variables. If ANOVA points out some feature combinations are significant, then that suggests the means of the groups of continuous variables are different. We chose 5% as the threshold for significance.

ANOVA makes some assumptions regarding the variances of the attributes which are not fully satisfied by all of our variables, meaning the results we obtained should be considered carefully.
...
We notice how the time signature variable shows significance in combination with all other variables. This may be due to the fact that this variable is highly imbalanced, since 93.4% of the songs have a time signature of 4/4. 

#### Partial Correlations

Because ANOVA does not inform us which means were different from the others, it is necessary to perform a Multiple Comparison Procedure. To do this, we utilized partial correlations, an exhaustive search that compares each possible pairing of continuous variables.
...
WHY ARE WE HIGHLIGHTING THIS OUTLIER? HOW DOES IT AFFECT THE PARTIAL CORRELATION? TALK ABOUT HOW WE LEFT IT IN THE DATA AS A PART OF OUR EXPLORATION AND THE LEARNING EXPERIENCE OF HAVING DATA WITH OUTLIERS.
...
To visualize these relationships, we utilize box plots:
...
To measure the independent/dependent variables association for the categorical variables, we utilize the Chi Squared Test. It will pass judgement on the level of independence between the variables.
...
The Chi Squared Test revealed fairly high p-values for all of the categorical variables, and even produced a 1 for mode suggesting complete independence of mode from the dependent variable. IS THIS CORRECT??
...
- Attempting to address the imbalance of the data
...
logistic_predictions_full = predict(logistic, newdata = test_set[,c(-1, -2)],
                                    type = "response")
```
...
Discuss what we see from this threshold compared to 0.2:
...
Discuss what we see from this threshold compared to 0.2 and 0.3
...
logistic_reduced = glm(IsWinner ~ danceability + loudness +  valence + duration_ms + acousticness, data = training_set,  family = "binomial")
...
- The purpose of the quantile-quantile (QQ) plot is to show if two datasets come from the same distribution or if there are deviations from a common distribution. It compares the quantiles (percentiles) of one dataset to the quantiles of another, typically to check for similarities or differences in their underlying distributions. 

In this case, we will be using  this type of plot for a normality analysis of the variables, comparing each of their distributions to a normal one with the same mean and standard deviation.
