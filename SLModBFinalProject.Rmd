...
```{r, fig.cap="Bar plots of the categorical variables"}
...
```{r, fig.cap="Log-likelihood of the Acousticness Box-Cox Parameter"}
par(mfrow = c(1, 1))
...
```{r, fig.cap="Log-likelihood of the Duration Box-Cox Parameter"}
...
```{r, fig.cap="Log-likelihood of the Energy Box-Cox Parameter"}
par(mfrow = c(1, 1))
...
```{r, fig.cap="Log-likelihood of the Instrumentalness Box-Cox Parameter"}
...
```{r, fig.cap="Log-likelihood of the Loudness Box-Cox Parameter"}
par(mfrow = c(1, 1))
...
```{r, fig.cap="Log-likelihood of the Tempo Box-Cox Parameter"}
...
```{r, fig.cap="Log-likelihood of the Valence Box-Cox Parameter"}
...
Future work could expand upon this research by consulting experts in the music industry, such as musicologists, to gain insight into other possible features. Adding new variables could allow the models to discover new connections they did not previously have and lead to higher classification abilities. Different types of models could also be considered, or modifications to the models considered here. Changing the accuracy metrics during the training and hyperparameter tuning stages of the KNN and/or early stopping could be implemented. Incorporating a larger data set with more recent data (if available) could also improve the models' abilities to classify nominated songs. Employing alternate, more advanced strategies to handle the imbalanced dataset could improve the output as well. Finally, inspecting analysis over time may be a useful perspective, offering insight into how trends over time may influence the likelihood of a type of song to be nominated.

Overall, the models produced in this project do offer some insight into what makes a rock song worthy of Grammy nomination, but cannot be reliably used to predict a nomination.