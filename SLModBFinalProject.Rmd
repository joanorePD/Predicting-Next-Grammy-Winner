...
The fact that these groups were significant allows us to say there may be a strong enough correlation between these pairings to use them in our models in a beneficial manner. DISCUSS HOW ALL WERE SIGNIFICANT WITH TIME SIGNATURE; WHY COULD THAT BE AND HOW DO WE ADDRESS THAT?
...
Because ANOVA does not inform us which means were different from the others, it is necessary to further explore the data pairings. To do this, we utilized partial correlations, an exhaustive search that compares each possible pairing of continuous variables.
...
There were three variable pairs that stood out as significantly different:
...
COMPARISON OF NON-NOMINATED / NOMINATED FOR A CONTINUOUS VARIABLE (VALENCE)

We also compared the distributions and means of each variable for non-nominated and nominated songs. Most were very similar, but below we highlighted the most different attribute: Valence.
...
hist(x, main = "Valence: Non-Nominated")
...
Depending on the dependent variable, the valence variable peaks at different locations and has a different mean. This could point towards valence being an important factor in our predictions.
...
Evident mode of the categories in Mode and Time Signature; VERY high frequency TALK ABOUT THIS
...
oversampled_train_data = ovun.sample(IsWinner ~., data = training_set[,-1],
...
- The purpose of the quantile-quantile (QQ) plot is to show if two data sets come from the same distribution (WRITEUP)
